...
const APP_BASE_URL = (
  process.env.NEXT_PUBLIC_APP_URL || 'https://4rtw0rk.vercel.app'
).replace(/\/+$/, '')
...
export async function generateMetadata({
  ...
  const dateParam = normalizeDate(searchParams.date)
  const url = buildDateUrl(dateParam)
  const image = buildOgImageUrl(dateParam)
